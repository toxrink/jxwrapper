...
object TimeUtils {
   ...
   * @return
   */
  def getNowBeforeByMinute(min: Int): Date = {
    val cal = Calendar.getInstance()
   ...
   * @return
   */
  def getNowBeforeByDate(date: Date, day: Int): Date = {
    val cal = Calendar.getInstance()
    cal.setTime(date)
  ...
  def getDays(start: Date, end: Date, offset: Int): Int = {
    if (null == start || null == end) {
      ...
      val day = BigDecimal(end.getTime - start.getTime) / BigDecimal(DAY_MS)
      if (0 == day) 1 else (day + offset).toInt
    }
  }
